    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            .Select(s => new SensorMapViewModel
            {
                ...
                LatestPM25 = s.AQIDataRecords.OrderByDescending(a => a.Timestamp).Select(a => (double?)a.PM25).FirstOrDefault(),
                LatestTimestamp = s.AQIDataRecords.OrderByDescending(a => a.Timestamp).Select(a => (DateTime?)a.Timestamp).FirstOrDefault(),
                TrendTimestamps = s.AQIDataRecords.Where(a => a.Timestamp >= now.AddHours(-24)).AsEnumerable()
                    .GroupBy(a => new DateTime(a.Timestamp.Year, a.Timestamp.Month, a.Timestamp.Day, a.Timestamp.Hour, 0, 0))
                    .OrderBy(g => g.Key)
                    .Select(g => g.Key)
                    .ToList(),
                TrendPM25 = s.AQIDataRecords.Where(a => a.Timestamp >= now.AddHours(-24)).AsEnumerable()
                    .GroupBy(a => new DateTime(a.Timestamp.Year, a.Timestamp.Month, a.Timestamp.Day, a.Timestamp.Hour, 0, 0))
                    .OrderBy(g => g.Key)
                    .Select(g => g.Average(x => x.PM25))
                    .ToList(),
                TrendPM10 = s.AQIDataRecords.Where(a => a.Timestamp >= now.AddHours(-24)).AsEnumerable()
                    .GroupBy(a => new DateTime(a.Timestamp.Year, a.Timestamp.Month, a.Timestamp.Day, a.Timestamp.Hour, 0, 0))
                    .OrderBy(g => g.Key)
